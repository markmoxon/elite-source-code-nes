...
; ******************************************************************************
;
;       Name: versionNumber_b2
;       Type: Variable
;   Category: Text
;    Summary: The game's version number in bank 2
;
; ******************************************************************************
...
;    Summary: Vectors and padding at the end of ROM bank 2
;  Deep dive: Splitting NES Elite across multiple ROM banks
;
; ******************************************************************************
...
IF _NTSC
 ...
 EQUW Interrupts_b2+$4000   ; Vector to the NMI handler in case this bank is
                            ; loaded into $C000 during start-up (the handler
                            ; contains an RTI so the interrupt is processed but
                            ; has no effect)

 EQUW ResetMMC1_b2+$4000    ; Vector to the RESET handler in case this bank is
                            ; loaded into $C000 during start-up (the handler
                            ; resets the MMC1 mapper to map bank 7 into $C000
                            ; instead)

 EQUW Interrupts_b2+$4000   ; Vector to the IRQ/BRK handler in case this bank is
                            ; loaded into $C000 during start-up (the handler
                            ; contains an RTI so the interrupt is processed but
                            ; has no effect)
